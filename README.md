...
The contract is **<50 ms p99** on inputs of `output ≤ 4k tokens` and `context ≤ 200,000 characters` (~50k tokens). The benchmark test in `tests/performance.test.ts` enforces this against a synthesized 200,000-character context on every `npm test` run. Coverage runs are exempted from the perf assertion because v8 coverage instrumentation adds ~2–3× overhead and would produce misleading numbers.
